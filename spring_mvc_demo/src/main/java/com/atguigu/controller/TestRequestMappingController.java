 ...
 * 作用：通过请求的请求路径匹配请求
 * value属性是数组类型，即当前浏览器所发送请求的请求路径匹配value属性中的任何一个值  value必须要有
 ...
 * 现在只有自己设置method为post才是post，其他的都为get方法。并且post只管表单，其他的归get管
 * 在@RequestMapping的基础上，结合请求方式的一些派生注解：
 * @GetMapping,@PostMapping,@DeleteMapping,@PutMapping
 * 4、@RequestMapping注解的params属性（除了value和method，后面的一般不用）
 ...
 * 5、@RequestMapping注解的headers属性（防止盗版小说，盗版视频网站的，防止用户手动输入网址进入，因为必须要有来源headers）
 ...
 * ?:任意的单个字符（不包括?）  实际应用输入网址的时候不能输入问号，因为？是路径和请求参数的分隔符
 * *:任意个数的任意字符（不包括?和/）
 ...
 * 在通过@PathVariable（路径中的变量）注解，将占位符所标识的值和控制器方法的形参进行绑定
 */
@Controller
//@RequestMapping("/test")  类上的map路径
public class TestRequestMappingController {

    //此时控制器方法所匹配的请求的请求路径为/test/hello
    @RequestMapping(
            value = {"/hello","/abc"},
            method = {RequestMethod.POST, RequestMethod.GET},//设置2种，两种都能访问
    ...
    @RequestMapping("/**/test/ant")//多层路径
    public String testAnt(){
        return "success";
    }

    @RequestMapping("/test/rest/{username}/{id}")//将路径中的id赋值给整数id
